...
### 4. 論文 英日対訳パイプライン（`factfull.bilingual`）

論文（arXiv ID / URL またはローカル PDF）を**完全英日対訳**の構造化 JSON にする。
段落単位で英日をペア化し、見出し・セクション階層を保持、図表は原文位置を保ったまま
画像抽出する。Scholaread の代替（オフライン・クォータなし）。homupe で記事化する際に
レイアウト（日英ブロックのフォント比率など）を自由に組み替えられる。

```bash
# arXiv ID から
python pipelines/bilingual.py 2403.11996

# ローカル PDF から（参考文献も残す・キャプション除外）
python pipelines/bilingual.py ~/papers/foo.pdf --keep-references --skip-captions
```

```python
from factfull.bilingual import BilingualConfig, run_bilingual

result = run_bilingual(BilingualConfig(model="translategemma:12b"), "2403.11996")
# result.json_path → ~/papers/bilingual/2403.11996/bilingual.json
#   { schema_version, title_en/ja, authors, blocks: [{id, type, en, ja, level,
#     section_path, page, bbox, image_path, label}], ... }
# 図表画像は ~/papers/bilingual/2403.11996/assets/ に保存
```

出力の各 `block` は入れ子なしの独立レコード（読み順）。homupe は `type`（title /
heading / abstract / paragraph / caption / reference / figure / table）と `level` だけで
レイアウトと図表配置を決められる。
...
└── bilingual/          # 論文 → 英日対訳 JSON パイプライン
    ├── extract.py      # pymupdf 構造抽出（テキスト＋図表）
    ├── segment.py      # 見出し/段落/図表/参考文献に整形
    ├── translate.py    # 段落バッチ翻訳（整列保証）
    ├── pipeline.py     # BilingualConfig / BilingualResult / run_bilingual()
    └── types.py        # BilingualDoc / Block
```
